...
class UraarticlesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        data = []
        with open('./uranews.json') as json_file:
        ...
        def parse(self, response):
            ...
            item['article_datetime'] = response.xpath(
                '//div[@class="item-info"]/time[@itemprop="datePublished"]/@datetime').extract()
            if not item['article_datetime']:
                item['article_datetime'] = response.xpath(
                    '//div[@class="publication-info"]/time[@itemprop="datePublished"]/@datetime').extract()
            ...
            item['article_title'] = response.xpath('//div[@class="item"]/h1/text()').extract()
            if not item['article_title']:
                item['article_title'] = response.xpath('//div[@class="publication"]/h1/text()').extract()

            item['article_text'] = "\n".join(response.xpath('//div[@class="item-text"]//p/text()').extract())
            # if not item['article_text']:
            #     item['article_text'] = "\n".join(response.xpath('//div[@class="item-text"]/div[3]//*/text()').extract())
            item['article_author'] = "\n".join(response.xpath('//ul[@class="item-authors"]//li/span/text()').extract())
